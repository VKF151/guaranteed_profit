...
public class ModRewardProvider implements DataProvider {
    private final FabricPackOutput output;
    private final String directory;
    private final Submitter[] submitters;

    public ModRewardProvider(FabricPackOutput output, String directory, Submitter... submitters) {
        this.output = output;
        ...
        return CompletableFuture.allOf(
                elements.entrySet().stream().map(x ->
                        DataProvider.saveStable(
                                cache,
                                x.getValue(),
                                paths.json(Identifier.fromNamespaceAndPath(Guaranteed_profit.MOD_ID, x.getKey()))
                        ))
